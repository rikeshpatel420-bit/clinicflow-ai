...
alter table public.calls
  add constraint calls_status_check check (
    status in ('missed', 'answered', 'recovered', 'voicemail', 'queued', 'failed', 'abandoned')
  );

alter table public.calls drop constraint if exists calls_recovery_status_check;
alter table public.calls
  add constraint calls_recovery_status_check check (
    recovery_status in ('not_started', 'queued', 'sms_draft', 'sms_sent', 'drafted', 'awaiting_reply', 'replied', 'booked', 'lost', 'recovered', 'closed', 'failed')
  );
...
alter table public.recovery_workflows
  add constraint recovery_workflows_state_check check (
    state in ('queued', 'sms_sent', 'replied', 'booked', 'lost', 'drafted', 'awaiting_staff_approval', 'message_queued', 'awaiting_patient_reply', 'closed', 'failed')
  );
